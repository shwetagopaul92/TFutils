...
Then check in EBI GWAS catalog.  We keep only chr17 for demonstration in
this package.

```{r dogwas}
GCRg = intersect(gcrpath$SYMBOL, TFutils::gwascat_hg19_chr17$MAPPED_GENE)
GCRg
```
...
library(dplyr)
library(magrittr)
chk = as(mcols(TFutils::gwascat_hg19_chr17), "data.frame") %>% 
...
Pure bed-like text will be split by chromosome and parsed directly.
Tabix-indexed bed can be imported with restriction specified by GRanges.
```{r lkaws}
importFIMO(system.file("M5946_1/chr1.bed", package="TFutils"))
vdf = system.file("M5946_1/chr1.bed.gz", package="TFutils")
importFIMO(Rsamtools::TabixFile(vdf), 
  GenomicRanges::GRanges("chr1", IRanges::IRanges(1e6,11e6)))
```
