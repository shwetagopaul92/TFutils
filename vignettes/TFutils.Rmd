...
rownames(mymat) = use
mymat[na.omit(TFs_GO@HGNCmap[,2]),1] = 1
mymat[na.omit(TFs_MSIG@HGNCmap[,2]),2] = 1
mymat[na.omit(TFs_HOCO@HGNCmap[-246,2]),3] = 1
mymat[na.omit(TFs_CISBP@HGNCmap[,2]),4] = 1
...
```{r dodo,echo=FALSE}
cismap = TFs_CISBP@HGNCmap
scis = split(cismap, cismap$HGNC)
uf = sapply(scis, function(x) x$Family_Name[1])
CISTOP = sort(table(uf),decreasing=TRUE)[1:10]
hoc = TFs_HOCO@HGNCmap
shoc = split(hoc, hoc$HGNC)
sfam = sapply(shoc, function(x)x$`TF family`[1])
